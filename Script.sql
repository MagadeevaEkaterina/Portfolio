...
ORDER BY payers_share DESC -- добавила сортировку по популярности у игроков
...
GROUP BY game_items),
CTE2 AS (
SELECT COUNT(transaction_id) AS total_purchases -- для упрощения основного запросе посчитаем количество всех совершенных покупок предметов
FROM fantasy.events),
CTE3 AS (
